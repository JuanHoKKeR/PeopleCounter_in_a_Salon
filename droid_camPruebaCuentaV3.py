...
def main():
    ...
    while cap.isOpened() is True:
        ret, frame = cap.read()
        if ret == False: break
        frame = imutils.resize(frame, height=300, width=533)
        #-------------------------------------------
        ...
        for cnt in cnts:
            ...
            if area > 500 and area < 2500:
                ...
                #-------------------------------------------
                sensor1 = verificacion(linea1, x, w, frame)
                sensor2 = verificacion(linea2, x, w, frame)
                sensor3 = verificacion(linea3, x, w, frame)
                #-------------------------------------------
                if sensor1:
